...
class KMEANS:
    def __init__(self, n_cluster, epsilon=1e-3, maxstep=2000):
        ...
        self.epsilon = epsilon  # mse小于epsilon，算法结束，mse=sse/n
        self.maxstep = maxstep  # 如果数据集较大，收敛困难时，迭代超过maxstep次，算法结束
        self.N = None
        self.centers = None
        self.cluster = defaultdict(list)  # 分簇结果

    def init_param(self, data):
        # 初始化参数, 包括初始化簇中心
        # 初始簇中心一般为随机点，不过此处选择了数据集中已有的点
        self.N = data.shape[0]
        random_ind = np.random.choice(self.N, size=self.n_cluster)
        self.centers = [data[i] for i in random_ind]  # 随机选取数据集中的点作为初始聚类中心，list存储中心点坐标数组
...
# 以下代码只有在单独执行k-means.py时才会执行，在其他地方import kmeans时不会执行
